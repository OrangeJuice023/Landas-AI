...
export function LiveJobs({ roleTitle }: { roleTitle: string }) {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      setJobs(await fetchLiveJobs(roleTitle));
    } catch (e) {
      setError(e instanceof Error ? e.message : "Couldn't load live jobs.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="mt-6">
      <h4 className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-400 mb-4">
        <Radar size={14} className="text-blue-500" /> Live PH Openings
      </h4>
        ...
        {jobs.map((job, i) => (
          ...
          >
            ...
            <div className="mt-auto pt-2 flex flex-wrap items-center gap-2">
              {job.salary && (
                <span className="px-2 py-0.5 bg-green-50 text-green-700 text-[10px] font-bold rounded-md">{job.salary}</span>
              )}
              {job.employment_type && (
                <span className="px-2 py-0.5 bg-gray-50 text-gray-500 text-[10px] font-bold rounded-md uppercase">{job.employment_type}</span>
              )}
              {job.posted_at && (
                <span className="text-[10px] text-gray-300 font-medium">{new Date(job.posted_at).toLocaleDateString()}</span>
              )}
            </div>
          </a>
        ))}
