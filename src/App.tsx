                ...
                <div className="flex flex-col lg:flex-row gap-8">
                  <div className="lg:w-1/3">
                    <div className="flex items-center gap-3 mb-4">
                       <span className="text-5xl font-black text-black/10">{(role.fit_score / 100 * 10).toFixed(1)}</span>
                       <div>
                         <h3 className="text-2xl font-bold leading-tight uppercase tracking-tight">{role.title}</h3>
                         <div className="flex items-center gap-1 mt-1 text-green-600 font-bold text-[10px] uppercase tracking-wider">
                            <Zap size={10} fill="currentColor" />
                            {role.fit_score}% Match Score
                         </div>
                       </div>
                    </div>
                    ...
                    {role.market_tags && (
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {role.market_tags.in_demand && <span className="px-2 py-0.5 bg-red-50 text-red-600 text-[9px] font-bold rounded-md border border-red-100 uppercase tracking-tighter">In Demand</span>}
                        {role.market_tags.remote_friendly && <span className="px-2 py-0.5 bg-blue-50 text-blue-600 text-[9px] font-bold rounded-md border border-blue-100 uppercase tracking-tighter">Remote</span>}
                        {role.market_tags.fresh_grad_friendly && <span className="px-2 py-0.5 bg-green-50 text-green-600 text-[9px] font-bold rounded-md border border-green-100 uppercase tracking-tighter">New Grad</span>}
                      </div>
                    )}
                    ...
                    {role.salary && (
                      <div className="mb-6 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                         <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Estimated PH Salary</p>
                         <div className="flex justify-between text-xs">
                            <span className="font-bold">{role.salary.entry}</span>
                            <span className="text-gray-300">/</span>
                            <span className="font-bold">{role.salary.mid}</span>
                         </div>
                      </div>
                    )}
                  ...
                  <div className="lg:w-2/3 grid md:grid-cols-2 gap-8 lg:border-l lg:border-gray-200 lg:pl-10">
                    <div>
                      <h4 className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-400 mb-4">
                        <Compass size={14} /> Career Roadmap
                      </h4>
                      <div className="space-y-3">
                        {role.career_path.map((p, idx) => (
                          <div key={idx} className="flex items-center gap-3">
                            <div className="w-1.5 h-1.5 rounded-full bg-black/20" />
                            <span className={idx === 2 ? "font-bold text-sm" : "text-sm text-gray-400"}>{p}</span>
                          </div>
                        ))}
                      </div>
                    </div>
